...
import {
  ...
  initialCases,
  type CaseDraft,
} from "@/components/gptool/CaseInput";
import { ReviewPanel } from "@/components/gptool/ReviewPanel";
import { GeneratingLoader } from "@/components/gptool/GeneratingLoader";
import {
...
export default function GpToolPage() {
  const [cases, setCases] = useState<CaseDraft[]>(initialCases);
  const [descriptorLevels, setDescriptorLevels] = useState<DescriptorLevel[]>([
    "competent",
  ]);
  ...
  const handleGenerate = async () => {
    ...
    const validCases = cases.filter(
      (c) => c.caseDescription.trim().length >= 20,
    );
    ...
    try {
      const response = await fetch("/api/generate-review", {
        ...
        body: JSON.stringify({
          cases: validCases.map((c) => ({
            ...
            letAiChoose: c.letAiChoose,
            selectedCapabilities: c.selectedCapabilities,
            capabilityMode: deriveCapabilityMode(
              c.letAiChoose,
      ...
      if (data.demo) {
        setDemoInfo(
          "Running in demo mode. Add OPENROUTER_API_KEY to .env.local, then restart the dev server.",
        );
      }
    } catch (err) {
      setError(
